...
local System = {
    __properties = {
        ...
        __last_ball_id = nil,
        __ping_label = nil,
        __auto_spam_mode = "Distance",  -- "Distance" atau "Speed"
        __auto_spam_distance = 30,
        __auto_spam_speed = 100
    },
    __config = {
...
-- ========== AUTO SPAM (Trigger Manual Spam otomatis) ==========
...
local function getClosestEnemy()
    local char = LocalPlayer.Character
    if not char or not char.PrimaryPart or not Alive then return nil end
    local myPos = char.PrimaryPart.Position
    local closest, minDist = nil, math.huge
    for _, entity in pairs(Alive:GetChildren()) do
        if entity ~= char and entity.PrimaryPart then
            local d = (myPos - entity.PrimaryPart.Position).Magnitude
            if d < minDist then
                minDist = d
                closest = entity
            end
...
local function shouldAutoSpam()
    if not System.__properties.__auto_spam_enabled then return false end
    local mode = System.__properties.__auto_spam_mode or "Distance"

    -- MODE DISTANCE: jarak kita ke musuh terdekat
    if mode == "Distance" then
        local enemy, dist = getClosestEnemy()
        if not enemy then return false end
        return dist <= (System.__properties.__auto_spam_distance or 30)
    end

    -- MODE SPEED: kecepatan bola yang menargetkan kita
    if mode == "Speed" then
        ...
        local speed = zoomies.VectorVelocity.Magnitude
        return speed >= (System.__properties.__auto_spam_speed or 100)
    end
...
local keybinds = {
    ...
    autoSpamKeyCode = Enum.KeyCode.Q,
    ...
    Callback = function(state)
        if System.__properties.__is_mobile then
            ...
            if state then
                System.manual_spam.start()
                if getgenv().ManualSpamNotify then
                    Fluent:Notify({Title="Manual Spam", Content="ON", Duration=2})
                end
            else
                System.manual_spam.stop()
                if getgenv().ManualSpamNotify then
                    Fluent:Notify({Title="Manual Spam", Content="OFF", Duration=2})
                end
            end
        end
    end
...
local auto_spam_section = Tabs.Spam:AddSection("Auto Spam", "repeat")

auto_spam_section:AddToggle("AutoSpamToggle", {
    Title = "Auto Spam",
    Description = "Aktifkan spam otomatis berdasarkan trigger",
    Default = false,
    Callback = function(value)
        System.__properties.__auto_spam_enabled = value
        if not value and System.__properties.__manual_spam_enabled then
            System.manual_spam.stop()
        end
        Fluent:Notify({Title="Auto Spam", Content=value and "ON" or "OFF", Duration=2})
    end
})

auto_spam_section:AddDropdown("AutoSpamMode", {
    Title = "Trigger Mode",
    Description = "Kapan auto spam aktif",
    Values = {"By Distance", "By Ball Speed"},
    Default = "By Distance",
    Multi = false,
    Callback = function(value)
        if value == "By Distance" then
            System.__properties.__auto_spam_mode = "Distance"
        elseif value == "By Ball Speed" then
            System.__properties.__auto_spam_mode = "Speed"
        end
        Fluent:Notify({Title="Auto Spam Mode", Content=value, Duration=1})
    end
})

auto_spam_section:AddSlider("AutoSpamDistance", {
    Title = "Max Distance",
    Description = "Jarak maksimal ke musuh terdekat (mode Distance)",
    Default = 30,
    Min = 5,
    Max = 200,
    Rounding = 0,
    Callback = function(value)
        System.__properties.__auto_spam_distance = value
    end
})

auto_spam_section:AddSlider("AutoSpamSpeed", {
    Title = "Min Ball Speed",
    Description = "Kecepatan minimum bola (mode Speed)",
    Default = 100,
    Min = 10,
    Max = 2000,
    Rounding = 0,
    Callback = function(value)
        System.__properties.__auto_spam_speed = value
    end
})
...
-- Auto Spam Keybind
keybinds_section:AddKeybind("AutoSpamKeybind", {
    Title = "Auto Spam Key",
    ...
    ChangedCallback = function(k)
        ...
        if keyName and keyName ~= "" then
            ...
            local kc = Enum.KeyCode[cleanName]
            if kc then 
                keybinds.autoSpamKeyCode = kc
                Fluent:Notify({Title="Auto Spam Key", Content=cleanName, Duration=2})
            end
        end
    end
})
...
UserInputService.InputBegan:Connect(function(input, gp)
    ...
    -- Auto Spam
    if kc == keybinds.autoSpamKeyCode then
        System.__properties.__auto_spam_enabled = not System.__properties.__auto_spam_enabled
        if not System.__properties.__auto_spam_enabled and System.__properties.__manual_spam_enabled then
            System.manual_spam.stop()
        end
        Fluent:Notify({Title="Auto Spam", Content=System.__properties.__auto_spam_enabled and "ON" or "OFF", Duration=1})
    end
